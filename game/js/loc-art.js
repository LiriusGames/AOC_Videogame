...
const LocArt = (() => {
  ...
  // ==================================================== paper-cutout rooms
  // The print-era room language (Pass 5, Print Floor = the approval sample):
  // a paper diorama — smooth flat shapes on warm stock, the user's engraving
  // cutout as the hero, and placed workers as die-cut paper dolls. The 240ms
  // ticker gives it stop-motion pose swaps, which IS the cutout feel.
  const CUTW = 672, CUTH = 448;
  const CUT = {
    paper: "#f2ead6", paper2: "#e6d8b8", floor: "#d9c9a4", floor2: "#c9b78e",
    ink: "#221d16", rim: "#f4eede", red: "#b5443a", brass: "#c9a26b",
  };
  const CUT_IMG = {};
  function cutSprite(name, dx, dy, w) {
    const a = ATLAS[name];
    if (!a || typeof SHEETS === "undefined") return;
    let img = CUT_IMG[a.sheet];
    if (!img) { img = new Image(); img.src = SHEETS[a.sheet].file; CUT_IMG[a.sheet] = img; }
    if (!img.complete || !img.naturalWidth) return;
    g.drawImage(img, a.x, a.y, a.w, a.h, dx, dy, w, w * a.h / a.w);
  }
  function rr(x, y, w, h, r, fill) {
    g.fillStyle = fill;
    g.beginPath(); g.roundRect(x, y, w, h, r); g.fill();
  }
  function cutShadow(x, y, rx, ry, a = 0.14) {
    g.fillStyle = `rgba(60,40,20,${a})`;
    g.beginPath(); g.ellipse(x, y, rx, ry, 0, 0, Math.PI * 2); g.fill();
  }
  // a printed sheet: white stock, thin ink frame, gray copy + a red masthead
  function cutSheet(x, y, w, h, rot = 0) {
    g.save(); g.translate(x, y); g.rotate(rot);
    rr(-w / 2 + 2, -h / 2 + 3, w, h, 3, "rgba(60,40,20,.12)"); // cut shadow
    rr(-w / 2, -h / 2, w, h, 3, "#fbf6ea");
    g.strokeStyle = "rgba(34,29,22,.5)"; g.lineWidth = 1.5;
    g.beginPath(); g.roundRect(-w / 2, -h / 2, w, h, 3); g.stroke();
    rr(-w / 2 + 5, -h / 2 + 5, w - 10, 7, 2, CUT.red);
    g.fillStyle = "rgba(34,29,22,.35)";
    for (let i = 0; i < 3; i++) rr(-w / 2 + 5, -h / 2 + 17 + i * 8, (w - 10) * (i === 2 ? 0.6 : 1), 3, 1.5, "rgba(34,29,22,.3)");
    g.restore();
  }
  // a die-cut paper doll: cream sticker rim, flat house wardrobe, the same
  // staffer identity quartet as the rail sprites (fedora/rolls/slick/curly)
  function cutDoll(x, y, ch, pub, o = {}) {
    const look = WORKER_LOOKS[ch];
    const at = workerAttire(ch, pub);
    const lean = o.jig ? 0.03 : -0.02;
    g.save(); g.translate(x, y); g.rotate(o.pose === "pin" ? lean * 0.6 : lean);
    // sticker underlay: one cream capsule silhouette behind the whole figure
    g.fillStyle = CUT.rim;
    g.beginPath(); g.roundRect(-26, -118, 52, 118, 24); g.fill();
    // legs
    if (at.dress) {
      g.beginPath(); g.moveTo(-16, -52); g.lineTo(16, -52); g.lineTo(22, -18); g.lineTo(-22, -18); g.closePath();
      g.fillStyle = at.skirtC; g.fill();
      rr(-11, -18, 7, 14, 3, look.skin); rr(4, -18, 7, 14, 3, look.skin);
      rr(-13, -6, 11, 6, 3, CUT.ink); rr(2, -6, 11, 6, 3, CUT.ink);
    } else {
      rr(-14, -46, 11, 42, 4, at.pants); rr(3, -46, 11, 42, 4, at.pants);
      rr(-16, -6, 14, 6, 3, CUT.ink); rr(2, -6, 14, 6, 3, CUT.ink);
    }
    // torso
    rr(-17, -88, 34, 42, 9, at.shirt);
    if (at.susp) { rr(-12, -88, 5, 38, 2, at.susp); rr(7, -88, 5, 38, 2, at.susp); }
    if (at.tie) rr(-3, -84, 6, 22, 3, at.tie);
    if (at.belt) rr(-17, -54, 34, 6, 3, at.belt);
    // arms + a sheet for the working poses
    g.strokeStyle = at.shirt; g.lineWidth = 9; g.lineCap = "round";
    if (o.pose === "hold") {
      g.beginPath(); g.moveTo(-14, -80); g.lineTo(-26, -64); g.stroke();
      g.beginPath(); g.moveTo(14, -80); g.lineTo(26, -64); g.stroke();
      cutSheet(0, -58, 34, 24, o.jig ? 0.05 : -0.03);
    } else if (o.pose === "pin") {
      g.beginPath(); g.moveTo(-12, -82); g.lineTo(-20, -104); g.stroke();
      g.beginPath(); g.moveTo(12, -82); g.lineTo(22, -102); g.stroke();
      g.fillStyle = look.skin;
      g.beginPath(); g.arc(-21, -106, 5, 0, Math.PI * 2); g.arc(23, -104, 5, 0, Math.PI * 2); g.fill();
    } else {
      g.beginPath(); g.moveTo(-14, -80); g.lineTo(-20, -56); g.stroke();
      g.beginPath(); g.moveTo(14, -80); g.lineTo(20, -56); g.stroke();
      g.fillStyle = look.skin;
      g.beginPath(); g.arc(-21, -53, 5, 0, Math.PI * 2); g.arc(21, -53, 5, 0, Math.PI * 2); g.fill();
    }
    // head
    g.fillStyle = look.skin;
    g.beginPath(); g.arc(0, -100, 14, 0, Math.PI * 2); g.fill();
    g.fillStyle = look.hair;
    if (look.style === "fedora") {
      g.beginPath(); g.ellipse(0, -110, 20, 5, 0, 0, Math.PI * 2); g.fill();
      rr(-12, -126, 24, 17, 6, o.hatC || pub.dark);
    } else if (look.style === "rolls") {
      g.beginPath(); g.arc(0, -106, 14, Math.PI, 0); g.fill();
      g.beginPath(); g.arc(-13, -98, 6, 0, Math.PI * 2); g.arc(13, -98, 6, 0, Math.PI * 2); g.arc(0, -117, 7, 0, Math.PI * 2); g.fill();
    } else if (look.style === "slick") {
      g.beginPath(); g.arc(0, -104, 14, Math.PI * 0.95, Math.PI * 2.05); g.fill();
    } else { // curly
      g.beginPath(); g.arc(-9, -110, 8, 0, Math.PI * 2); g.arc(0, -114, 9, 0, Math.PI * 2);
      g.arc(9, -110, 8, 0, Math.PI * 2); g.arc(-13, -100, 6, 0, Math.PI * 2); g.arc(13, -100, 6, 0, Math.PI * 2); g.fill();
    }
    g.fillStyle = CUT.ink;
    g.beginPath(); g.arc(-5, -100, 1.8, 0, Math.PI * 2); g.arc(5, -100, 1.8, 0, Math.PI * 2); g.fill();
    g.restore();
  }
  // the five print-floor stations, prime seats first (catching the fresh
  // sheets, stacking, pinning the drying line, proofing, watching)
  const PRINT_STATIONS = [
    { x: 330, y: 330, pose: "hold" },
    { x: 218, y: 336, pose: "hold" },
    { x: 122, y: 322, pose: "pin" },
    { x: 52, y: 330, pose: "idle" },
    { x: 168, y: 340, pose: "idle" },
  ];
  function drawPrintCut(f) {
    // warm stock backdrop, a whisper of halftone, floor band
    let gr = g.createLinearGradient(0, 0, 0, 300);
    gr.addColorStop(0, CUT.paper); gr.addColorStop(1, CUT.paper2);
    g.fillStyle = gr; g.fillRect(0, 0, CUTW, 300);
    gr = g.createLinearGradient(0, 300, 0, CUTH);
    gr.addColorStop(0, CUT.floor); gr.addColorStop(1, CUT.floor2);
    g.fillStyle = gr; g.fillRect(0, 300, CUTW, CUTH - 300);
    g.fillStyle = "rgba(34,29,22,.05)";
    for (let y = 10; y < 290; y += 14)
      for (let x = 8 + (y % 28 ? 7 : 0); x < CUTW; x += 14) {
        g.beginPath(); g.arc(x, y, 1.3, 0, Math.PI * 2); g.fill();
      }
    g.fillStyle = CUT.ink; g.fillRect(0, 299, CUTW, 2);
    // the drying line: rope + today's sheets, swaying on alternate beats
    g.strokeStyle = "rgba(34,29,22,.65)"; g.lineWidth = 2;
    g.beginPath(); g.moveTo(14, 74); g.quadraticCurveTo(170, 96, 330, 56); g.stroke();
    for (let i = 0; i < 4; i++) {
      const px = 48 + i * 76, py = 80 + Math.sin((px / 330) * 2.4) * 10;
      const sway = ((f + i) % 2 ? 1 : -1) * 0.035;
      g.fillStyle = CUT.ink; g.fillRect(px - 2, py - 6, 4, 8); // pin
      cutSheet(px, py + 36, 46, 62, sway);
    }
    // ink drums, far left on the floor
    cutShadow(58, 334, 44, 8);
    rr(28, 270, 34, 62, 6, "#5b6a8a"); rr(64, 282, 30, 52, 6, "#6e6a3f");
    rr(28, 270, 34, 8, 4, "#4a5875"); rr(64, 282, 30, 7, 4, "#5c592f");
    g.fillStyle = CUT.rim; g.font = "bold 11px sans-serif"; g.textAlign = "center";
    g.fillText("INK", 45, 305); g.fillText("INK", 79, 313);
    // fresh stack: the day's run piles up left of the press
    cutShadow(252, 342, 58, 9);
    for (let i = 0; i < 5; i++) rr(222, 330 - i * 7, 62, 8, 3, i % 2 ? "#fbf6ea" : "#efe6d0");
    g.strokeStyle = "rgba(34,29,22,.4)"; g.lineWidth = 1;
    g.strokeRect(222, 330 - 28, 62, 36);
    // the hero: the user's pressman engraving, thumping on the beat
    const thump = f % 4 < 2 ? 0 : 3;
    cutShadow(500, 336, 128, 14, 0.18);
    cutSprite("hd_vig_print", 350, 66 + thump, 300);
    // a fresh sheet flies from the press mouth to the stack (3 stop-motion poses)
    const fly = f % 3;
    cutSheet(392 - fly * 60, 268 + fly * 22, 56, 40, -0.12 + fly * 0.05);
    // your workers, as die-cut dolls at the stations
    const occ = roomOccupants("print");
    occ.forEach((oc, i) => {
      const st = PRINT_STATIONS[i % PRINT_STATIONS.length];
      const p = UI.engine.player(oc.pid), pub = PUBLISHERS[p.color];
      const ch = staffCharFor("print", oc.slot);
      cutShadow(st.x, st.y + 4, 26, 6);
      cutDoll(st.x, st.y, ch, pub, { pose: st.pose, jig: (f + i) % 2 === 0 });
    });
    // paper-diorama corner shade (drawn, not the pixel rooms' black frame)
    gr = g.createRadialGradient(CUTW / 2, 190, 240, CUTW / 2, 190, 480);
    gr.addColorStop(0, "rgba(60,40,20,0)"); gr.addColorStop(1, "rgba(60,40,20,.22)");
    g.fillStyle = gr; g.fillRect(0, 0, CUTW, CUTH);
  }
  const CUT_PAINTERS = { print: drawPrintCut };
  ...
  function attach(cv, action) {
    if (CUT_PAINTERS[action]) {
      // paper-cutout rooms render SMOOTH (die-cut paper, not pixels)
      cv.width = CUTW; cv.height = CUTH; cv.classList.add("cut-scene");
      start(cv, CUT_PAINTERS[action]);
    }
    else if (HD_PAINTERS[action]) { cv.width = HDW; cv.height = HDH; start(cv, HD_PAINTERS[action]); }
